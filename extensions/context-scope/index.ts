...
import { isJsonObject, positiveNumberValue, stringValue, type JsonValue } from "./lib/boundary.ts";
import { findContainerBy, isResourceRow, RESOURCE_HEADER_RE, type ContainerLike } from "./lib/chat.ts";
import { configPaths, expandHomePath, readJsonConfig } from "./lib/config.ts";
import { compactCount } from "./lib/fmt.ts";
import { ELLIPSIS, GLYPH, SEP, ink, panelHeader } from "./lib/style.ts";
...
type ContextScopeTui = {
  children?: unknown[];
  requestRender?: (force?: boolean) => void;
};

type ContextScopeGlobal = typeof globalThis & {
  __piContextScopeTui?: ContextScopeTui;
  __piContextScopeChat?: ContainerLike;
  __piContextScopeBlock?: StartupContextComponent;
  __piContextScopeMode?: ViewMode;
  __piContextScopeInstallTimer?: ReturnType<typeof setTimeout>;
  __piContextScopeModel?: ModelSummary;
};

const g = globalThis as ContextScopeGlobal;
...
function compactPath(filePath: string): string {
  const home = homedir();
  if (filePath === join(getAgentDir(), "AGENTS.md")) return "Global AGENTS.md";
...
function mergeContextScopeConfig(base: ContextScopeConfig, next?: ContextScopeConfig): ContextScopeConfig {
  if (!next) return base;
  return {
    ...base,
    ...next,
...
function parseContextScopeConfig(value: unknown): ContextScopeConfig {
  if (!isJsonObject(value)) return {};
  const config: ContextScopeConfig = {};
  if (isJsonObject(value.defaults)) {
    const defaults: NonNullable<ContextScopeConfig["defaults"]> = parseHeuristicProfile(value.defaults);
...
function loadContextScopeConfig(cwd: string): ContextScopeConfig {
  const paths = [...configPaths("pi-contextscope", cwd), ...splitConfigPaths(process.env.PI_CONTEXTSCOPE_CONFIG)];
  return paths.reduce<ContextScopeConfig>(
    (config, filePath) => mergeContextScopeConfig(config, readJsonConfig(filePath, parseContextScopeConfig)),
    {},
  );
}
...
function resolveHeuristic(model: ModelSummary | undefined, config: ContextScopeConfig): ResolvedHeuristic {
...
function renderHeader(snapshot: PrefixSnapshot, mode: ViewMode, theme: Theme): string[] {
  ...
  return panelHeader(theme, "ContextScope", {
...
class StartupContextComponent implements Component {
  readonly __piContextScopeBlock = true;
  ...
  render(width: number): string[] {
    try {
      ...
      this.cachedSignature = "contextscope-unavailable";
      this.cachedMode = this.mode;
      this.cachedWidth = width;
      this.cachedLines = wrapLines([
        "",
        `${accent(undefined, "[ContextScope]")} unavailable while Pi finishes resuming this session`,
      ], Math.max(20, width));
      return this.cachedLines;
    }
  }
...
function isPrefixBlock(component: unknown): component is StartupContextComponent {
  return !!component && typeof component === "object" && (component as { __piContextScopeBlock?: boolean }).__piContextScopeBlock === true;
}

// _lib/chat.ts isResourceRow, minus the panel itself: the [ContextScope] block renders
...
function isContextBlockInstalled(block: StartupContextComponent): boolean {
  const chat = g.__piContextScopeChat;
...
function installContextBlock(block: StartupContextComponent): boolean {
  const tui = g.__piContextScopeTui;
  const chat = findResourceChatContainer(tui) ?? g.__piContextScopeChat;
  if (!chat || !Array.isArray(chat.children)) return false;

  g.__piContextScopeChat = chat;
...
function scheduleInstall(block: StartupContextComponent): void {
  if (g.__piContextScopeInstallTimer) clearTimeout(g.__piContextScopeInstallTimer);
  let attempts = 0;
  const attempt = () => {
    attempts++;
    if (safely(() => installContextBlock(block), false)) return;
    if (attempts < 30) {
      g.__piContextScopeInstallTimer = setTimeout(attempt, 50);
    }
  };
  g.__piContextScopeInstallTimer = setTimeout(attempt, 0);
}

function setMode(mode: ViewMode): void {
  g.__piContextScopeMode = mode;
  g.__piContextScopeBlock?.setMode(mode);
  g.__piContextScopeTui?.requestRender?.(true);
}
...
export default function piContextScope(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const currentMode = g.__piContextScopeMode ?? DEFAULT_MODE;
    const config = loadContextScopeConfig(ctx.cwd);
    g.__piContextScopeModel = toModelSummary(ctx.model);
    markDirty();
    ...
    g.__piContextScopeBlock = block;

    ctx.ui.setWidget("__pi_contextscope_capture", (tui: ContextScopeTui) => {
      g.__piContextScopeTui = tui;
      return {
        render: () => {
          const activeBlock = g.__piContextScopeBlock;
  ...
  pi.on("model_select", async (event, ctx) => {
    g.__piContextScopeModel = toModelSummary(event.model);
    markDirty();
    g.__piContextScopeBlock?.invalidate();
    if (ctx.hasUI) g.__piContextScopeTui?.requestRender?.(true);
  });

  pi.on("session_shutdown", async () => {
    if (g.__piContextScopeInstallTimer) clearTimeout(g.__piContextScopeInstallTimer);
    g.__piContextScopeInstallTimer = undefined;
  });

  pi.registerCommand("contextscope", {
    description: "Show or switch the startup [ContextScope] view (summary, compact, expanded)",
    handler: async (args, ctx) => {
      ...
      const mode: ViewMode = requested === "summary" || requested === "compact" || requested === "expanded"
        ? requested
        : nextMode(g.__piContextScopeMode ?? g.__piContextScopeBlock?.getMode() ?? DEFAULT_MODE);
      setMode(mode);
      if (g.__piContextScopeBlock) scheduleInstall(g.__piContextScopeBlock);
      ctx.ui.notify(`[ContextScope] view: ${mode}`, "info");
    },
  });
}
