...
OAuth account credentials refresh through WorkOS. Configured API-key mode stores only a non-secret selector in Pi auth; actual keys remain in the permission-restricted `<agent-dir>/factory/api-keys.json`. Direct API-key mode stores the key through Pi's normal credential storage.
