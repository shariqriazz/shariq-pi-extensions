...
function createParentBridgeTools(task: SpawnTask): ToolDefinition[] {
  ...
  return [
    {
      ...
      parameters: Type.Object({
        message: Type.String({ description: "Parent update" }),
      }),
    ...
    },
    {
      ...
      parameters: Type.Object({
        question: Type.String({ description: "Parent question" }),
      }),
      async execute(_toolCallId, params, signal) {
        const { question } = params as { question: string };
        const reply = await bridge.ask(question, signal);
    ...
    },
    {
      ...
      async execute(_toolCallId, params) {
        ...
        await bridge.messagePeer(id, message);
        return { content: [{ type: "text", text: `Message sent to ${id}.` }], details: { id } };
      },
    },
  ] as ToolDefinition[];
}
      ...
      try: async () => {
        ...
        const sessionManager = task.resumeSessionFile
          ...
          : SessionManager.create(task.cwd, undefined, {
              parentSession: task.parent.parentSessionFile,
            });
        if (!task.resumeSessionFile) {
          for (const message of task.parent.inheritedMessages ?? []) {
        ...
        const { session } = await createAgentSession({
          ...
          model,
          thinkingLevel,
          customTools: createParentBridgeTools(task),
          excludeTools: [
            ...CHILD_EXCLUDED_TOOL_NAMES,
            ...capabilityExclusions(task.capability),
          ],
        });
