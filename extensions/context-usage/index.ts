...
type ContextUsageTui = {
  children?: unknown[];
  requestRender?: (force?: boolean) => void;
};

type ContextUsageGlobal = typeof globalThis & {
  __piContextUsageTui?: ContextUsageTui;
  __piContextUsageChat?: ContainerLike;
  __piContextUsageBlock?: StartupContextComponent;
  __piContextUsageMode?: ViewMode;
  __piContextUsageInstallTimer?: ReturnType<typeof setTimeout>;
  __piContextUsageModel?: ModelSummary;
};
...
function mergeContextUsageConfig(base: ContextUsageConfig, next?: ContextUsageConfig): ContextUsageConfig {
  if (!next) return base;
  return {
    ...base,
    ...next,
...
function parseContextUsageConfig(value: unknown): ContextUsageConfig {
  if (!isJsonObject(value)) return {};
  const config: ContextUsageConfig = {};
  if (isJsonObject(value.defaults)) {
    const defaults: NonNullable<ContextUsageConfig["defaults"]> = parseHeuristicProfile(value.defaults);
...
function splitConfigPaths(value: string | undefined, separator = delimiter): string[] {
...
function resolveHeuristic(model: ModelSummary | undefined, config: ContextUsageConfig): ResolvedHeuristic {
...
function renderHeader(snapshot: PrefixSnapshot, mode: ViewMode, theme: Theme): string[] {
  ...
  return panelHeader(theme, "Context Usage", {
...
class StartupContextComponent implements Component {
  readonly __piContextUsageBlock = true;
...
function isPrefixBlock(component: unknown): component is StartupContextComponent {
  return !!component && typeof component === "object" && (component as { __piContextUsageBlock?: boolean }).__piContextUsageBlock === true;
}

// _lib/chat.ts isResourceRow, minus the panel itself: the [Context Usage] block renders
...
function isContextBlockInstalled(block: StartupContextComponent): boolean {
  const chat = g.__piContextUsageChat;
...
function scheduleInstall(block: StartupContextComponent): void {
  if (g.__piContextUsageInstallTimer) clearTimeout(g.__piContextUsageInstallTimer);
  let attempts = 0;
  const attempt = () => {
    attempts++;
  ...
  g.__piContextUsageInstallTimer = setTimeout(attempt, 0);
}

function setMode(mode: ViewMode): void {
  g.__piContextUsageMode = mode;
  g.__piContextUsageBlock?.setMode(mode);
  g.__piContextUsageTui?.requestRender?.(true);
}
...
export default function piContextUsage(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    g.__piContextUsageBlock = block;

    ctx.ui.setWidget("__pi_context_usage_capture", (tui: ContextUsageTui) => {
      g.__piContextUsageTui = tui;
      return {
        render: () => {
          const activeBlock = g.__piContextUsageBlock;
  ...
  pi.on("model_select", async (event, ctx) => {
    g.__piContextUsageModel = toModelSummary(event.model);
    markDirty();
    g.__piContextUsageBlock?.invalidate();
    if (ctx.hasUI) g.__piContextUsageTui?.requestRender?.(true);
  });

  pi.on("session_shutdown", async (_event, ctx) => {
    if (g.__piContextUsageInstallTimer) clearTimeout(g.__piContextUsageInstallTimer);
    g.__piContextUsageInstallTimer = undefined;
    const block = g.__piContextUsageBlock;
    const chat = g.__piContextUsageChat;
    if (block && chat) {
    ...
    if (ctx.hasUI) {
      ctx.ui.setWidget("__pi_context_usage_capture", undefined);
      g.__piContextUsageTui?.requestRender?.(true);
    }
    g.__piContextUsageBlock = undefined;
    g.__piContextUsageChat = undefined;
    g.__piContextUsageTui = undefined;
    g.__piContextUsageModel = undefined;
  ...
  pi.registerCommand("context-usage", {
    description: "Show or switch the startup [Context Usage] view (summary, compact, expanded)",
    handler: async (args, ctx) => {
      ...
      const mode: ViewMode = requested === "summary" || requested === "compact" || requested === "expanded"
        ? requested
        : nextMode(g.__piContextUsageMode ?? g.__piContextUsageBlock?.getMode() ?? DEFAULT_MODE);
      setMode(mode);
      if (g.__piContextUsageBlock) scheduleInstall(g.__piContextUsageBlock);
      ctx.ui.notify(`[Context Usage] view: ${mode}`, "info");
    },
  });
}
