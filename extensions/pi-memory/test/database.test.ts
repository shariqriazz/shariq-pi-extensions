...
test("automatic extraction cannot resurrect an explicitly forgotten memory", () => {
  const { root, database, project } = fixture();
  try {
    const memory = database.addMemory({ candidate: candidate(), projectId: project.id, sourceKind: "manual", sourceRef: "manual" });
    assert.equal(database.forgetMemory(memory.memoryId), true);
    const automatic = database.addMemory({ candidate: candidate(), projectId: project.id, sourceKind: "pi-session", sourceRef: "session" });
    assert.equal(automatic.memoryId, memory.memoryId);
    assert.equal(automatic.status, "deleted");
    assert.equal(database.countMemories(), 0);

    const explicit = database.addMemory({ candidate: candidate(), projectId: project.id, sourceKind: "manual", sourceRef: "manual-restore" });
    assert.equal(explicit.memoryId, memory.memoryId);
...
test("correction collisions fail instead of reporting another memory as corrected", () => {
  const { root, database, project } = fixture();
  try {
    const first = database.addMemory({ candidate: candidate({ content: "First distinct fact." }), projectId: project.id, sourceKind: "manual", sourceRef: "first" });
    const second = database.addMemory({ candidate: candidate({ content: "Second distinct fact." }), projectId: project.id, sourceKind: "manual", sourceRef: "second" });
    assert.throws(
      () => database.updateMemory(first.memoryId, candidate({ content: second.content }), project.id, "manual", "correction"),
      new RegExp(`duplicate active memory ${second.memoryId}`),
    );
    assert.equal(database.getMemory(first.memoryId)?.content, "First distinct fact.");
  } finally {
    database.close();
    fs.rmSync(root, { recursive: true, force: true });
  }
...
test("revisiting an existing branch job updates the capture watermark", () => {
  const { root, database, project } = fixture();
  try {
    const base = {
      sessionId: "branch-session", sessionFile: "/session.jsonl", project,
      entryIds: ["entry"], transcript: "transcript", queryText: "query", capturedAt: Date.now(),
    };
    assert.equal(database.enqueueCapture({ ...base, leafId: "leaf-a" }), true);
    assert.equal(database.enqueueCapture({ ...base, leafId: "leaf-b" }), true);
    assert.equal(database.getCaptureCheckpoint(base.sessionId), "leaf-b");
    assert.equal(database.enqueueCapture({ ...base, leafId: "leaf-a" }), false);
    assert.equal(database.getCaptureCheckpoint(base.sessionId), "leaf-a");
  } finally {
    database.close();
    fs.rmSync(root, { recursive: true, force: true });
  }
