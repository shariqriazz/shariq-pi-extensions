...
import { hasSensitiveToolArguments, redactSecrets, sanitizeToolArguments } from "./redaction.ts";
...
function serializeEntry(entry: EntryLike, sensitiveToolCallIds: ReadonlySet<string>): string | undefined {
  ...
  if (role === "toolResult") {
    ...
    if (toolCallId && sensitiveToolCallIds.has(toolCallId)) return undefined;
...
export function captureSession(
  ...
  for (const entry of branch) {
    ...
    for (const rawBlock of entry.message.content) {
      ...
      if (
        ...
        && hasSensitiveToolArguments(block.name, block.arguments)
      ) {
        sensitiveToolCallIds.add(block.id);
      }
    }
  }
  ...
  const newSerialized = branch.slice(firstNewIndex)
    .map((entry) => ({ entry, text: serializeEntry(entry, sensitiveToolCallIds) }))
  ...
  const serialized = selected.map((entry) => serializeEntry(entry, sensitiveToolCallIds)).filter((value): value is string => Boolean(value));
