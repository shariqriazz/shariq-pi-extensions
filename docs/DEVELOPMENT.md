...
mise exec --locked -- npm pack --dry-run --workspace @shariqriazz/pi-memory
```
...
2. Run `pi install npm:@shariqriazz/pi-extensions`; do not reload yet.
...
npm publish --access restricted --workspace @shariqriazz/pi-memory
```
