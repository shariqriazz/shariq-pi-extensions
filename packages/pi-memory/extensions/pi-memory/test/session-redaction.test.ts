...
import { redactSecrets, sanitizeToolArguments } from "../src/redaction.ts";
...
test("captures only the active Pi branch and does not serialize thinking", () => {
  const entries = [
    ...
    { type: "message", id: "a1", message: { role: "assistant", content: [
      ...
      { type: "toolCall", name: "read", arguments: { path: "/tmp/.env" } },
    ] } },
    { type: "message", id: "t1", message: { role: "toolResult", toolName: "read", isError: false, content: [{ type: "text", text: "api_key=abcdef-secret-value" }] } },
  ...
  assert.deepEqual(captured.entryIds, ["u1", "a1", "t1"]);
